...
const StitchedAllProducts = () => {
  ...
  // STITCHED CATEGORY ID 
  const stitchedCategoryId = useSelector(state => state.category.categories.find(category => category.name === "Stitched")?.id);

  // FILTER ALL PRODUCT OF STITCHED CATEGORY
  const StitchedProducts = allProducts.filter((product) => product.category === stitchedCategoryId);


  // GET ALL CATEGORIES-TYPES OF STITCHED CATEGORY
  useEffect(() => {
    if (stitchedCategoryId) {
      dispatch(getCategoryTypeAsync({ category: stitchedCategoryId }));
    }
  }, [stitchedCategoryId, dispatch]);


  // GET ALL CATEGORY-TYPE HERE
  ...
  useEffect(() => {
    dispatch(
      getProductAsync({ currentPage, limit, category: stitchedCategoryId })
    ).then((response) => {
      setTotalPages(response.payload.productData.totalPages);
    });
  }, [dispatch, currentPage, limit, stitchedCategoryId]);
  ...
  const filterbyPrice = (range) => {
    ...
    if (range === "Low To High") {
      productsByPrice = StitchedProducts
        .slice()
  ...
  const filterbyDiscount = () => {
    const discunted = StitchedProducts.filter(
    ...
    <>
      <section className="StitchedAllProducts py-4 my-3">
        <div className="StitchedAllProducts-cont">
          ...
          <div className="all-product-body mb-5 pb-5">
